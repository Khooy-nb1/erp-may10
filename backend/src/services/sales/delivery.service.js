...
const { dispatchFulfillmentDelivery } = require('./fulfillment.service');
...
function createDeliveryService(repository = deliveryRepository, fulfillmentFn = dispatchFulfillmentDelivery) {
  ...
  async function completeDelivery(id, updaterId) {
    ...
    // Parity / smoke compatibility mode for legacy isolated Phase 1 test suites
    if (delivery.ghi_chu && (delivery.ghi_chu.includes('[SMOKE-PARITY]') || delivery.ghi_chu.includes('[SMOKE]'))) {
    ...
    return fulfillmentFn(id, updaterId);
  }
