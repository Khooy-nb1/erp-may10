 ...
 * (`0100109106` or `0100109106-001`). The value is stored verbatim, so the
 * uniqueness index in `database/schema.sql` compares exactly what is validated.
 */
...
const createCustomerSchema = v
  .object({
    ...
    // Optional, but when present it must be a Vietnamese tax id and must not
    // collide with another customer (checked in the service, enforced by the
    // partial unique index `uq_khach_hang_ma_so_thue`).
    ma_so_thue: v
      .string()
      .trim()
    ...
    email: v
      ...
      .nullable()
      .or(v.literal('')),
    dia_chi: v.string().trim().min(5, 'Địa chỉ phải từ 5 ký tự').max(500, 'Địa chỉ tối đa 500 ký tự'),
    tinh_thanh_pho: v
      .string()
      .trim()
      .min(2, 'Tỉnh/thành phố phải từ 2 ký tự')
...
function createCustomerService(repository = customerRepository) {
   ...
   * friendly 409; the partial unique index `uq_khach_hang_ma_so_thue` is the
   * hard guarantee when two requests race (mapped below from the PG error).
   */
  ...
  async function updateCustomer(id, rawInput, updaterId) {
    ...
    // A tax id may be re-sent unchanged; only a value taken by *another*
