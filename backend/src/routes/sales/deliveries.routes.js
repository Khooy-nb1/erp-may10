...
const { dispatchFulfillmentDelivery } = require('../../services/sales/fulfillment.service');
...
function createDeliveryRoutes(service = deliveryService) {
  ...
  // 5b. Fulfill delivery (PH1 -> PH4 fulfillment orchestration) - admin & kho ONLY
  router.post('/:id/fulfill', requireRoles('admin', 'kho'), async (req, res, next) => {
    try {
      const userId = (req.user && req.user.id) || 1;
      const updated = await dispatchFulfillmentDelivery(parseIdParam(req.params.id, 'id', 'Mã đơn giao hàng'), userId);
      sendSuccess(res, updated, 'Xuất kho và hoàn thành đợt giao hàng thành công.');
    } catch (err) {
      next(err);
    }
  });
