...
/**
 * Transaction-aware: Locks delivery row FOR UPDATE using the provided client.
 * @param {object} client
 * @param {number} id
 * @returns {Promise<object|null>}
 */
async function findByIdForUpdate(client, id) {
  const sql = `
    SELECT g.id, g.ma_giao_hang, g.ma_don_ban_hang, g.ma_kho,
           g.ngay_giao, g.ten_nguoi_nhan, g.dia_chi_giao,
           g.phuong_tien_van_chuyen, g.nguoi_giao_hang,
           g.ghi_chu, g.trang_thai
    FROM giao_hang g
    WHERE g.id = $1
    FOR UPDATE
  `;
...
/**
 * Transaction-aware: Finds full delivery record with joined tables.
 * @param {object} clientOrDb
 * @param {number} id
 * @returns {Promise<object|null>}
 */
async function findFullDeliveryById(clientOrDb, id) {
...
/**
 * Transaction-aware: Locks order lines FOR UPDATE and joins san_pham.
 * @param {object} client
 * @param {number} orderId
 * @returns {Promise<object[]>}
 */
async function getOrderLinesForFulfillment(client, orderId) {
  const sql = `
    SELECT l.id, l.ma_don_ban_hang, l.ma_san_pham,
           sp.ma_san_pham AS ma_san_pham_code, sp.ten_san_pham,
           sp.gia_von, sp.gia_ban,
           l.so_luong, l.don_gia, l.ty_le_giam_gia, l.thanh_tien,
           COALESCE(l.so_luong_giao, 0) AS so_luong_giao,
           (l.so_luong - COALESCE(l.so_luong_giao, 0)) AS so_luong_con_lai,
           l.ghi_chu, l.trang_thai
...
/**
 * Transaction-aware: Increments so_luong_giao and updates line status.
 * @param {object} client
 * @param {number} lineId
 * @param {number} addedQty
 * @returns {Promise<object|null>}
 */
async function updateOrderLineDeliveredQty(client, lineId, addedQty) {
  const sql = `
    UPDATE chi_tiet_don_ban_hang
    SET so_luong_giao = so_luong_giao + $1,
        trang_thai = CASE
          WHEN (so_luong_giao + $1) >= so_luong THEN 'da_giao_du'
          WHEN (so_luong_giao + $1) > 0 THEN 'giao_mot_phan'
          ELSE trang_thai
        END
    WHERE id = $2
    RETURNING *
  `;
  const res = await client.query(sql, [addedQty, lineId]);
  return res.rows[0] || null;
}

/**
 * Transaction-aware: Updates delivery status inside a transaction.
 * @param {object} client
 * @param {number} id
 * @param {string} newStatus
 * @param {number} updaterId
 ...
 * @returns {Promise<object|null>}
 */
async function updateDeliveryStatusWithClient(client, id, newStatus, updaterId, extra) {
  const fields = ['trang_thai = $2', 'nguoi_cap_nhat = $3', 'ngay_cap_nhat = NOW()'];
  const params = [id, newStatus, updaterId];
...
module.exports = {
  ...
  findByIdForUpdate,
  findFullDeliveryById,
  getOrderLinesForFulfillment,
  updateOrderLineDeliveredQty,
  updateDeliveryStatusWithClient,
};
