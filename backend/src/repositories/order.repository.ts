...
import { CalculatedOrderLine } from '../utils/pricing.js';
...
export interface IOrderRepository {
  ...
  update(
    id: number,
    headerUpdates: Partial<Pick<OrderRecord, 'ngay_giao_hang_yc' | 'dia_chi_giao_hang' | 'ghi_chu' | 'tong_tien_hang' | 'tien_thue' | 'tien_giam_gia' | 'tong_thanh_toan'>>,
    lines: CalculatedOrderLine[] | null,
    updaterId: number
  ...
  updateStatus(
    ...
    status: OrderStatus,
    updaterId: number,
    extraFields?: { ngay_giao_thuc_te?: Date | null; ghi_chu?: string | null }
  ): Promise<OrderRecord | null>;
  getCustomerOutstanding(customerId: number): Promise<number>;
}
...
export class OrderRepository implements IOrderRepository {
  async list(filters: OrderListFilters): Promise<OrderListResult> {
    ...
    let sortColumn: OrderSortColumn = 'ngay_dat_hang';
    if (filters.sortBy && ALLOWED_ORDER_SORT_COLUMNS.includes(filters.sortBy as OrderSortColumn)) {
      sortColumn = filters.sortBy as OrderSortColumn;
    }
    ...
    const dataResult = await query<OrderRecord>(dataSql, params);
  ...
  async findById(id: number): Promise<OrderRecord | null> {
    const orderSql = `
      ...
      LIMIT 1
    `;
    const orderRes = await query<OrderRecord>(orderSql, [id]);
    ...
    const linesSql = `
      ...
      ORDER BY l.id ASC
    `;
    const linesRes = await query<OrderLineRecord>(linesSql, [id]);
  ...
  async findByCode(code: string): Promise<OrderRecord | null> {
    const orderSql = `
      ...
      LIMIT 1
    `;
    const orderRes = await query<OrderRecord>(orderSql, [code.trim()]);
    return orderRes.rows[0] || null;
  }

  async create(params: CreateOrderParams): Promise<OrderRecord> {
    return withTransaction(async (client) => {
      ...
      const headerSql = `
        ...
        RETURNING *
      `;
      const headerRes = await client.query<OrderRecord>(headerSql, [
        params.ma_don_ban,
      ...
      const insertedLines: OrderLineRecord[] = [];
      for (const line of params.lines) {
        const lineSql = `
          ...
          RETURNING *
        `;
        const lineRes = await client.query<OrderLineRecord>(lineSql, [
          order.id,
          line.ma_san_pham,
          line.so_luong,
          line.don_gia,
  ...
  async update(
    ...
    return withTransaction(async (client) => {
      // 1. Update header
      const fields: string[] = ['nguoi_cap_nhat = $2', 'ngay_cap_nhat = NOW()'];
      const params: unknown[] = [id, updaterId];
      ...
      const updateHeaderSql = `
        ...
        RETURNING *
      `;
      const headerRes = await client.query<OrderRecord>(updateHeaderSql, params);
      const order = headerRes.rows[0];
      if (!order) return null;
  ...
  async getCustomerOutstanding(customerId: number): Promise<number> {
    const sql = `
      SELECT COALESCE(SUM(so_tien_con_lai), 0) AS total_outstanding
      FROM cong_no
      WHERE ma_khach_hang = $1 AND loai_cong_no = 'phai_thu'
    `;
    const res = await query<{ total_outstanding: string | number }>(sql, [customerId]);
