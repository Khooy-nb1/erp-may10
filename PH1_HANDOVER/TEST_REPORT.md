...
|---|---|---|---|
| `test_ph1_sales_api.js` | PH1 API smoke/regression: auth gates, envelopes, validation, write matrix, Core/PH4 reachability | `node tests/test_ph1_sales_api.js` | **115 passed / 0 failed** |
...
Frontend (Core workspace): `npm ci` (210 packages) + `npm run build` → **exit 0**, 1815 modules, single `index-*.js` bundle (see `docs/ph1-remediation/STEP10_BUILD_EVIDENCE.md`).
...
| Parameterized SQL / no injection | parity (`sortBy` injection attempt + table integrity) |
| FK integrity | cross-module TEST 08, audit script |
| Strict validation + field-level `details` | smoke suite, parity (10 validation scenarios) |
| Duplicate documents | smoke suite (invoice 409), document-code formats in parity |
| Mass assignment | parity (`PATCH` whitelist) |
