...
export default function MrpPage({ showToast }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-blue-50/60 rounded-xl border border-blue-100 text-xs text-blue-900 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="leading-relaxed">
            ...
            2. <strong>Tồn kho khả dụng</strong> = Tổng tồn kho thực tế từ Phân hệ Kho PH4, cộng theo từng kho từ bảng dùng chung `ton_kho.so_luong_ton`.<br />
      ...
      {stockDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={() => setStockDetail(null)}>
          <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{stockDetail.ma_vat_tu} · Tổng tồn: <strong>{new Intl.NumberFormat('vi-VN').format(stockDetail.tong_ton_kho || 0)}</strong></p>
              </div>
              <button onClick={() => setStockDetail(null)} className="p-2 rounded-lg hover:bg-slate-100"><X className="w-4 h-4" /></button>
            </div>
            <div className="p-5">
              {stockLoading ? (
                <div className="py-10 text-center text-sm text-slate-500">Đang lấy dữ liệu tồn kho từ PH4...</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {(stockDetail.warehouses || []).map((row) => (
                        <tr key={row.ma_kho}>
                          <td className="px-3 py-2 font-mono">{row.ma_kho_code || row.ma_kho}</td>
                          <td className="px-3 py-2 font-medium">{row.ten_kho}</td>
                          <td className="px-3 py-2 text-right font-bold">{new Intl.NumberFormat('vi-VN').format(row.so_luong_ton)} {row.don_vi_tinh || ''}</td>
                          <td className="px-3 py-2 text-slate-500">{row.ngay_cap_nhat ? new Date(row.ngay_cap_nhat).toLocaleString('vi-VN') : '—'}</td>
                        </tr>
                      ))}
                      {(stockDetail.warehouses || []).length === 0 && (
                        <tr><td colSpan="4" className="px-3 py-8 text-center text-slate-400">Chưa có tồn kho cho vật tư này.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            <div className="px-5 py-3 bg-slate-50 border-t border-slate-200 text-[11px] text-slate-500">
              Dữ liệu được đọc từ bảng <code>ton_kho</code> của PH4. Phân hệ Sản xuất không tự ý thay đổi tồn kho.
            </div>
          </div>
        </div>
      )}
