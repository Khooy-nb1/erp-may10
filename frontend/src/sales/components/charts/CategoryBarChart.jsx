...
import { buildValueAxis, clampRatio } from './chartScale.js';
...
const PADDING = { top: 20, right: 12, bottom: 34, left: 56 };
...
const SKELETON_COLUMN_RATIOS = [0.55, 0.85, 0.4, 0.7, 0.95];
const SKELETON_LEGEND_ROWS = [0, 1, 2, 3];

/**
 * @typedef {object} CategoryPoint
 * @property {string} key
 * @property {string} label
 * @property {number} value
 * @property {string} [color] Explicit fill, a colour the theme module does not know about.
 */

/**
 * @typedef {object} CategoryBarChartProps
 * @property {CategoryPoint[]} points
 * @property {(value: number) => string} formatValue
 * @property {string} ariaLabel
 * @property {(value: number) => string} [formatAxis] Short formatter for the y-axis ticks; falls back to formatValue.
 * @property {string} [valueSuffix] Unit shown under the category axis, e.g. "đơn".
 * @property {number} [height]
 * @property {boolean} [showShare]
 ...
 * @property {string} key
 * @property {string} label
 * @property {string} shortLabel Clipped for the axis; the full text stays in the column's <title>.
 * @property {number} value
 * @property {string} color
 * @property {number} share Exact percentage, so the stacked strip still sums to 100.
 * @property {number} x
 * @property {number} width
 * @property {number} y
 * @property {number} height
 * @property {boolean} showLabel False when the column is too narrow to print a label without collisions.
 ...
 * @property {{ value: number, ratio: number }[]} ticks
 * @property {number} total
 * @property {number} plotWidth
    ...
    <div ref={ref} className={cn('relative w-full', className)}>
            ...
            return (
              <g key={tick.ratio}>
                ...
                <text
                  ...
                  className="fill-brand-secondary text-[11px]"
                >
            ...
            <g key={column.key}>
              ...
              {index === activeIndex && (
                <rect
                  ...
                  rx={6}
                  className="fill-brand-light stroke-brand-primary"
                  strokeOpacity={0.35}
                  strokeWidth={1}
                  vectorEffect="non-scaling-stroke"
                />
              ...
              {column.showValueLabel && (
                <text
                  ...
                  textAnchor="middle"
                  className="fill-brand-text text-[11px] font-semibold tabular-nums"
                >
      ...
      {showShare && (
        <div className="mt-3 flex flex-col gap-3">
          <div className="flex h-1.5 w-full overflow-hidden rounded-full bg-slate-50">
          ...
          <ul className="flex flex-col gap-1">
            {layout.columns.map((column) => (
              <li key={column.key} className="flex items-center gap-2 text-sm">
                ...
                />
                <span className="min-w-0 flex-1 truncate text-brand-text">{column.label}</span>
                <span className="tabular-nums text-brand-text">{formatValue(column.value)}</span>
      ...
      <details className="mt-3">
        ...
        <table className="mt-2 w-full border-collapse text-sm">
          <thead>
            <tr className="text-left text-xs text-brand-secondary">
          ...
          <tbody>
            {layout.columns.map((column) => (
              <tr key={column.key} className="border-t border-brand-border">
                <td className="py-1.5 text-brand-text">{column.label}</td>
                <td className="py-1.5 text-right tabular-nums text-brand-text">
                  {formatValue(column.value)}
                </td>
                <td className="py-1.5 text-right tabular-nums text-brand-secondary">
                  {SHARE_FORMAT.format(column.share)}%
                </td>
              </tr>
            ))}
      ...
      {activeColumn && width > 0 && (
        <div
          aria-hidden
          className="pointer-events-none absolute z-10 flex flex-col gap-0.5 whitespace-nowrap rounded-xl border border-brand-border bg-white px-3 py-2 text-sm shadow-md"
          style={{
            left: `${Math.min(
              TOOLTIP_MAX_PERCENT,
              Math.max(
        ...
        >
          <span className="font-medium text-brand-text">{activeColumn.label}</span>
...
export const CategoryBarChartSkeleton = ({ height = DEFAULT_HEIGHT, className }) => (
