...
import { Text, Heading } from '../components/ui/Typography.jsx';
...
import { hasAnyRole, FINANCE_ROLES } from '../config/permissions.js';

/**
 * Accounts receivable, read-only inside the Sales module (PH1
 * \`pages/receivables/ReceivableListPage.tsx\`, ported 1:1 for Step 6H).
 *
 * The aging report stays behind Core's \`accounting.receivable\` scope (owner ruling R-2):
 * the client only issues \`GET /sales/cong-no/aging\` for admin/ke_toan, so a legitimate
 * load never produces a 403. Core's RBAC context replaces PH1's own context import.
 */
...
export function ReceivableListPage() {
  const { user } = useAuth();
  // GET /receivables/aging is restricted to the finance roles (ruling R-2); ban_hang must never request it.
  const canViewAging = hasAnyRole(user, FINANCE_ROLES);
  ...
  const [sortBy, setSortBy] = useState('ngay_dao_han');
   ...
   */
  const renderSortHeader = (column, label, tooltip) => (
    <Button
      aria-label={
        sortBy === column ? label + ' (sắp xếp ' + (sortOrder === 'ASC' ? 'tăng dần' : 'giảm dần') + ')' : label
      }
      variant="ghost"
      size="sm"
      icon={
  ...
  const columns = [
      ...
      renderCell: (item) => (
        <div className="flex flex-col gap-0.5">
          ...
          {item.ma_khach_hang_code ? (
            <Text variant="code" className="text-brand-secondary">
              {item.ma_khach_hang_code}
            </Text>
          ) : null}
        </div>
    ...
    },
    {
      ...
      align: 'end',
      renderCell: (item) => (
        <Text className="font-bold tabular-nums">{formatCurrency(item.so_tien_con_lai)}</Text>
      ),
    },
    {
      key: 'ngay_dao_han',
