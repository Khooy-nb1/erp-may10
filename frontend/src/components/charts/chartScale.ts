...
export interface ValueTick {
  value: number;
  /** Position on the value axis, 0 at the baseline and 1 at the top. */
  ratio: number;
}

export interface ValueAxis {
  /** Upper bound of the value axis; callers scale their geometry against this, not a separate nice number. */
  bound: number;
  ticks: ValueTick[];
}
 ...
 */
export function buildValueAxis(max: number, count = 4): ValueAxis {
...
export function buildLinePath(points: ScalePoint[]): string {
...
export function buildAreaPath(points: ScalePoint[], baselineY: number): string {
  if (points.length === 0) return '';
