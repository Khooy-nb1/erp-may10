...
export interface TrendPoint {
  ...
  label: string;
  value: number;
  /** Optional secondary line shown in the tooltip, e.g. "3 hóa đơn". */
  caption?: string;
}

export interface TrendChartProps {
  points: TrendPoint[];
  /** Full-precision formatter for tooltips and the data table. */
  formatValue: (value: number) => string;
  /** Short formatter for the y-axis labels; falls back to formatValue. */
...
const SURFACE_PAINT = 'var(--color-surface)';
const FOCUS_PAINT = 'var(--color-primary)';
 ...
 */
function buildLabelIndices(points: TrendPoint[], plotWidth: number): Set<number> {
...
function labelsFit(points: TrendPoint[], slot: number, left: number, right: number): boolean {
...
export const TrendChart: React.FC<TrendChartProps> = ({
  ...
  const handleKeyDown = (event: React.KeyboardEvent<SVGRectElement>): void => {
    if (count === 0) return;
      ...
      {count > 0 && width > 0 ? (
        <>
          ...
          {activePlot !== null && tooltipLeft !== null ? (
            <div
              className="pointer-events-none absolute z-10 rounded-control border border-border bg-surface px-3 py-2 text-sm shadow-raised"
              style={{
                left: `${tooltipLeft}%`,
            ...
            >
              <div className="text-muted-foreground">{activePlot.point.label}</div>
              <div className="font-medium tabular-nums">
                {formatValue(activePlot.point.value)}
              </div>
              {activePlot.point.caption ? (
                <div className="text-muted-foreground">{activePlot.point.caption}</div>
              ) : null}
            </div>
          ) : null}
        </>
      ...
      {count > 0 ? (
        <details className="mt-3">
          ...
          <table className="mt-2 w-full text-sm">
            <thead>
              <tr>
                <th scope="col" className="pb-1 text-left font-medium text-muted-foreground">
                  Kỳ
                </th>
                <th scope="col" className="pb-1 text-right font-medium text-muted-foreground">
                  {seriesLabel}
                </th>
              </tr>
            </thead>
...
export const TrendChartSkeleton: React.FC<{ height?: number; className?: string }> = ({
  height = DEFAULT_HEIGHT,
  className,
}) => (
